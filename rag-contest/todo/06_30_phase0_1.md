...
### [ ] 단계 2: 실험 기록 스키마 + 로거 (`harness/recorder.py`)
...
### [ ] 단계 3-1: FAA PDF → 깨끗한 텍스트 추출 (`harness/extract_faa.py`)
...
### [ ] 단계 3-2: §·part 태깅 + 파서 테스트 (`harness/extract_faa.py`)
...
### [ ] 단계 4: FAA 인덱싱 + §·part 메타 (`indexer.py`)
...
### [ ] 단계 5: 답변 Sources에 § 인용 노출 (`backend/app.py`)
...
- _(예: part67 표가 2단 레이아웃이라 pdfplumber가 열을 섞음 → `extract_words`+x좌표 정렬로 보정)_
