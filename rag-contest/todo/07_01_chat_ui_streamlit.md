...
### [x] 단계 0: 대회 규정 확인 — **Streamlit 허용 확인 (2026-07-01)** ✅

**결론**: **프레임워크 교체 허용** → Streamlit로 진행 확정.
- 근거 ①: 사용자가 대회측 확인 — "스트림릿 사용 가능해".
- 근거 ②: `CONTEST.md` 루브릭은 **행동**(Answer Quality·Citations·Cost·Clarity·UX·Robustness)만 채점, 프레임워크 지정 없음. "Everyone builds a RAG chat" = 방식 자유.
- 근거 ③: `STRATEGY.md`상 rag-starter는 **베이스**("바닥부터 안 만들어도 된다")지 필수 산출물 아님.
- → [`06_30_chat_ui.md`](06_30_chat_ui.md)(React)는 폴백으로 보존만.
...
### [x] 단계 3: 테마 정돈 (크림+teal 인상) ✅

> 완료(2026-07-01): 제목 세리프(D6)·태그라인·빈화면 인사(D5). **§ 조항 terracotta**(안전한 Streamlit `:orange`, 모델출력에 raw HTML 안 넣음=주입 방어). 사용자 요청 반영: `[n]`을 **`[n] §조항`으로 인라인 확장**(마커 옆에 근거 조항 바로 보임, 연속인용 안 깨짐 확인). config.toml 테마색은 앞서 단계1때 앞당김.
> 참고: 이로써 "인용 §번호 본문에 다 쓰기"는 **UI가 표시로 해결** → 출력 세션 프롬프트 변경 불필요.
...
### [x] 단계 4: UX 3종 + 멀티턴 (스트리밍·검색먼저·후속칩) ✅ (칩 제외)

> 완료(2026-07-01): **4a 스트리밍**(토큰 실시간 → 완료 시 서식본 스냅)·**4b 검색먼저**(§ 찾음 줄)·**4c 멀티턴**(과거 Q/A만 전달, 과거 CONTEXT 미재전송=E2) + **검색 문맥화**(직전 질문 이어붙여 짧은 후속 "night?"도 §91.151 회수 — 무료). **4d 후속칩은 스킵**(정적=캔느낌, 생성형=Cost 15 손해; 빈화면 예시버튼이 시작 유도는 커버). 브라우저 눈확인: 스트리밍 체감속도↑·멀티턴 야간 45분 정답·비용 안 부풀음.
