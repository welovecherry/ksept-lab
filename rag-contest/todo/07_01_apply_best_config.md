...
- **final holdout 눈검증 → 출전 (2026-07-01):** 배포본(bge/vector/K5 + B_balanced)으로 **final 4문제(H05·H08·H10·H13)를 딱 한 번** 생성·확인. **통과 ✅ (3.5/4)**: H05·H08 정확(검색 1/1), H13 정확히 거부, H10은 검색이 §61.23(의료) 미포착(1/2)했으나 모델이 **환각 없이 "출처에 없음" 정직 고지 + §61.57 완전답** — 그라운딩·부분답 규칙이 실전 작동. 인용오류 0건. **과적합 아님**(final이 tune과 일관). H10 §61.23 미검색은 알려진 검색 한계(coverage 0.818)로 기록만 — **final에 맞춰 재튜닝하지 않음**(§수정6). → **출전 가능.**
