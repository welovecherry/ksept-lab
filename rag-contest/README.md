...
> 미국 연방항공규정(14 CFR)을 근거로 답하는 **RAG 챗봇**이다. 질문 1건을 **단발(single-shot)** 과 **에이전틱(agentic)** 두 방식으로 나란히 응답하고, 실제 조항 번호(예: `§91.151`)로 인용한다. 대회 출품용으로 개발했다.

**개요.** 오픈북 시험과 같이, 모델은 관련 법 조항을 검색해 읽고 **그 근거로만** 답한다. 이로써 환각을 방지하고 인용을 검증 가능하게 한다.

---

## 📸 데모

질문 1건에 대해 **🎯 단발 / 🤖 에이전틱** 응답을 나란히 비교한다.
...
모델(학생)은 항공법을 사전에 학습하지 않는다. 대신 검색기(사서)가 질문과 관련된 조항을 선별해 제공하면, 모델은 **제공된 조각만 근거로** 답을 작성하고 출처를 인용한다.

- **RAG** = Retrieval(검색) + Augmented Generation(생성). "먼저 검색하고, 그 근거로 생성한다"는 구조다.
- **청크(chunk)** = 긴 법 원문을 검색 단위로 자른 조각. 전체 코퍼스는 **2,184개** 청크로 구성된다.
...
> 초기에는 단발만 구현했고, 이후 에이전트를 추가했다. 현재는 두 방식을 함께 제공한다. 대회의 비용 항목을 고려해 단발을 기본값으로 두되, 범위가 넓은 질문에서는 에이전트가 우위를 가진다.
...
핵심 차이는 통제권에 있다. 단발은 코드가 검색을 1회 통제하고, 에이전틱은 모델이 "추가 검색 여부"와 "응답 시점"을 스스로 결정한다.

---

## 🏆 검색 챔피언

임베딩 4종 × 청킹 2종 × 검색 3종 × K 3종 = **45개 조합**을 11개 문제로 채점했다. 결과는 다음과 같다.
...
- **§ 단위 청킹**: 법령은 조항 단위로 완결되므로, 글자수보다 조항 경계 분할이 유리하다.
- **bge 임베딩**: 영어 검색에 특화된 1024차원 모델로, 작은 K에서도 정답을 상위에 배치해 토큰을 절감한다.
...
> 키는 `.env`에만 보관하며 커밋하지 않는다. 최초 실행 시 bge 모델을 메모리에 적재하므로 지연이 발생한다(재다운로드가 아님).
